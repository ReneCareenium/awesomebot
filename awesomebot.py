...
async def help(ctx):
    ...
    admintext=('\n'+
            '$start: closes the signup phase and prepares the files for this tournament'+
            ...
            '$kick <id>: Kicks player <id> from the tournament'+
            '$standings: get the standings table\n'+
            '$pairings: compute and preview pairings. Will inform you of unfinished games beforehand.\n' +
...
async def join(ctx, url, rank=None):
    ...
    await ctx.send(f"Player {displayname} ({ogsname}) joined with rank {rank}!")
